...
- **CodeBLEU is NOT used as a quality metric** — superseded by CodeBERTScore (semantic) + CrystalBLEU (n-gram with trivial-grams stripped). Our actual primary signals are selector quality + smell deltas + AST non-triviality (ts-morph / tree-sitter + Zhang-Shasha) — see the confidence formula above and `scripts/evaluate.ts`. **Execution-based mutation-kill-rate is planned, not yet implemented** (no mutation-testing harness runs today; tracked in `ROADMAP.md`).
