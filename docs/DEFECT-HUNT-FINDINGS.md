...
| DEF2 | **Cross-app POM contamination** — batch/sequential migrations against different apps share-overwrite a shared POM; one migration's spec then references another app's locators, green by every static gate. | `validate-pom-provenance.ts` flags the UNAMBIGUOUS case (one POM carrying >1 distinct plan ref). The proposed "orphan" check was REJECTED after running it: it false-positives on legitimate pwm-blueprint reuse. Wired into the wall. | ✅ committed |
