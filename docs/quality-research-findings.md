...
| 2 | **POM-contamination conformance (W15/W16 + W1 amend)** — ban content assertions in shared `waitForPageLoad()`; flag multi-migration POM authorship; action methods assert *transitions*, not business outcomes | M | no | `validate-qa-master-conformance.ts` `listPageMethods()`, `migration-rules.md:74-81` | POM contamination |
