...
12. **Report metrics paraphrased from the plan instead of counted from your emitted code.** When you write the `## Metrics` section, you MUST count selectors, assertions, and locator-quality buckets **from the `.spec.ts` you just wrote**, not copy the plan's estimates. Verify-CR lens flags this specifically (PR #15 FluentWait FIX FIRST: report claimed "1 canonical / 1 fragile = 50%" for a spec with exactly 1 locator — arithmetic impossible). Concrete rules:
    ...
    - **The report's `Output:` line MUST reference YOUR emitted spec filename — kebab-cased — verbatim**. Three failure modes seen in practice:
        1. **Wholesale copy-paste from another report** (PR #13, 2026-06-09): report said `Output: outputs/tests/using_selenium_tests.spec.ts (29 LOC)` while the actual emitted spec was `outputs/tests/prompt-jupiter.spec.ts (52 LOC)`.
        2. **Snake-case input filename copied through unchanged** (sel-python iter 6, 2026-06-10): input `using_selenium_tests.py` → emitted spec correctly at `outputs/tests/using-selenium-tests.spec.ts` (kebab-case per migration-rules §"File naming") → report INCORRECTLY said `Output: outputs/tests/using_selenium_tests.spec.ts`. The report MUST mirror the actually-emitted kebab-case filename, NOT the input's snake_case. Same rule applies to PascalCase input (`FluentWaitJupiterTest.java` → emitted at `fluent-wait-jupiter-test.spec.ts`, NOT `FluentWaitJupiterTest.spec.ts`).
        3. **Snake-case `.ts` extension on Python source** (related): `_my_test.py` migrates to `my-test.spec.ts`, NOT `_my_test.spec.ts`.

        Before writing the `Output:` line, look at the file you actually wrote (or are about to write) under `outputs/tests/`, and copy its kebab-cased basename verbatim. Run `wc -l` mentally on your own emitted code before writing the LOC line. The `validate-report-metrics.ts` validator hard-fails on snake_case/PascalCase carryover.
