...
- Does the file structure match `migration-rules.md` §1–§4 (v0.2.0 pwm-blueprint)? Spec at `outputs/tests/`, all helpers at `outputs/helper/{fixtures,page-object/{pages,blocks},api,actions,utilities,test-data,types}/`. Spec imports `test` + `expect` from `@fixtures/base.fixture` only — `@playwright/test` imports outside `base.fixture.ts` itself are a `block`-severity finding (KB `pwm-blueprint/architecture/import-source`). Specs calling `page.goto()` directly are also `block` (KB `pwm-blueprint/architecture/page-goto-in-spec`) — navigation lives on the Page's `open()`. Legacy v0.1.x migrations at `outputs/tests/{pages,fixtures}/` are acceptable history; new migrations must be pwm-blueprint-shaped.
