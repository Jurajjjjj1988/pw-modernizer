...
- Did the plan say "no POM"? Under v0.2.0 pwm-blueprint architecture, **every migration still needs at least one PageClass** (the spec cannot call `page.goto()` directly per `pwm-blueprint/architecture/page-goto-in-spec`). "No POM" in legacy v0.1.x envelopes is reinterpreted as "the minimum scaffold only" — base.fixture + basepage + a single thin PageClass. Gold-plating with multi-level POMs/blocks the plan didn't ask for is still a finding (severity warn).
