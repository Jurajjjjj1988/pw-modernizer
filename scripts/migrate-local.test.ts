...
test("derivePaths: BASE/plan/envelope/report mirror migrate.yml (BASE = basename(input))", () => {
  const p = derivePaths({ input: "inputs/bad-playwright/foo.spec.ts", plan: "", mock: false, help: false });
...
test("derivePaths: explicit --plan overrides the default plan path", () => {
  const p = derivePaths({ input: "inputs/cypress/bar.cy.js", plan: "custom/plan.md", mock: false, help: false });
