...
validator (`scripts/validate-qa-master-conformance.ts`) must reject as
block-severity violations. (1) **Two** `await page.waitForTimeout(...)`
calls in the spec — `qa-master/runtime/no-hard-waits`, caught by Check 8;
...
`qa-master/architecture/page-goto-in-spec`, caught by Check 7; the selenium
...
selector priority — `qa-master/page-object/locator-priority`, caught by W5
